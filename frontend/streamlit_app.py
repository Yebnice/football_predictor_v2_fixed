...
if not fixtures:
    ...
    <div style="background: var(--background-card-alt); border: 1px solid var(--border-color); border-radius: 10px; padding: 1rem; margin: 1rem 0;">
        <strong>Prediction output is generated by the background ML pipeline.</strong>
        <span style="color: var(--text-secondary); margin-left: 0.35rem;">
            The button below loads the latest statistical-model predictions that have already passed the AI review gate and builds the 5-slip package.
        </span>
    </div>
    """, unsafe_allow_html=True)
